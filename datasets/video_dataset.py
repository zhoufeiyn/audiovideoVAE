...
class VideoAudioClipDataset(Dataset):
    ...
    def __init__(self, cfg):
        ...
        # ---- filtering ----
        self.video_paths = self._scan_videos(self.root)
        ...
        if cfg.min_video_frames > 0 and _HAS_DECORD:
            filtered = []
    ...
    # Audio segment read
    # ------------------------
    def _read_audio_segment(self, path: str, start_sec: float, duration_sec: float) -> torch.Tensor:
        if not self.load_audio or (duration_sec <= 0):
        ...
        try:
            ...
            wav, sr = torchaudio.load(path)  # wav: (channels, num_samples)
            ...
            import warnings
            warnings.warn(f"Audio decode failed for {path}: {type(e).__name__}: {str(e)}")
            return torch.zeros((1, 0), dtype=torch.float32)
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        audio = self._read_audio_segment(path, start_sec=start_sec, duration_sec=duration_sec)
