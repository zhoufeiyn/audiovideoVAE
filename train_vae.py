...
from models.vae2_2 import WanVAE_  # assumes you run from repo root: Wan2.2/
...
def make_model(cfg: TrainVAEConfig, device: torch.device) -> nn.Module:
...
def main():
    ...
    logger = setup_logging(cfg.out_dir, name="wan22vae")
    logger.info("===== Wan2.2 VAE Training =====")
    logger.info(f"device: {'cuda' if torch.cuda.is_available() else 'cpu'}")
    logger.info(f"out_dir: {cfg.out_dir}")
    logger.info(f"data root: {getattr(cfg, 'root', None)}")
    ...
    if cfg.resume:
        ckpt = torch.load(cfg.resume_ckpt_path, map_location="cpu")
        model.load_state_dict(ckpt["model"], strict=True)
        optim.load_state_dict(ckpt["optim"])
        start_step = int(ckpt.get("step", 0))
        logger.info(f"[resume] loaded {cfg.resume_ckpt_path} @ step={start_step}")
    else:
    ...
    for step in pbar:
        ...
        # ---- save ----
        if (step + 1) % cfg.save_every == 0 and cfg.out_dir:
            ckpt_path = os.path.join(cfg.out_dir, f"ckpt_step{step + 1}.pth")
            ...
            with torch.no_grad():
                ...
                save_video(x_rec_vis, os.path.join(cfg.out_dir, f"x_rec_{step + 1}.mp4"))
                save_video(x_vis, os.path.join(cfg.out_dir, f"x_{step + 1}.mp4"))
                logger.info(f"[save] videos -> x_rec_{step + 1}.mp4, x_{step + 1}.mp4 (T={T_vis})")

    # final save
    if cfg.out_dir:
        save_ckpt(os.path.join(cfg.out_dir, "ckpt_final.pth"), model, optim, cfg.max_steps)
